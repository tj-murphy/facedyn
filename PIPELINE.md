...
| 2 | Train/test split | 80/20 split, preserving real/fake video pairing | `final_analysis_NMF_check.Rmd` ~L284 | ✅ **Done** — `src/facedyn/splitting.py`, two functions: `paired_train_test_split` (replicates this paper's exact matched-pairs design) and `group_train_test_split` (general-purpose default for any study design, thin wrapper around `sklearn.model_selection.GroupShuffleSplit`) — see `DATA_FORMAT.md` for which one fits a given dataset | **Stochastic**, not deterministic-once-seeded as originally assumed here: R's `sample()` and NumPy's RNG select different videos even with a "matched" seed, so `paired_train_test_split` is validated structurally instead — unit tests check the pairing invariant/proportions/determinism on synthetic data; a real-data validation test (`tests/validation/test_splitting_validation.py`, skipped if `Data/` absent) confirms the split on the actual dataset produces exactly 370 train / 94 test videos, matching the R markdown's reported counts. `group_train_test_split` has no R counterpart (added for general reusability, not part of the paper) so only unit-tested, no R comparison needed |
